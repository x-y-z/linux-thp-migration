				...
				int nr_pages, enum migrate_mode mode)
{
	...
	for (i = 0; i < nr_pages; ) {
		...
		if (rc && (mode & MIGRATE_MT))
			rc = copy_page_mt(dst, src, 1);
						...
						enum migrate_mode mode)
{
	...
	if (accel_page_migration &&
		!(mode & (MIGRATE_DMA|MIGRATE_MT))) {
		mode |= MIGRATE_DMA;
		mode |= MIGRATE_MT;
	}
					   ...
					   enum migrate_mode mode)
{
	...
	if (PageHuge(page) || PageTransHuge(page))
		...
		if (rc && (mode & MIGRATE_MT))
			rc = copy_page_mt(newpage, page, 1);
		...
		int __user *, status, int, flags)
{
	...
	/* Check flags */
	if (flags & ~(MPOL_MF_MOVE|MPOL_MF_MOVE_ALL|MPOL_MF_MOVE_DMA|MPOL_MF_MOVE_MT))
		return -EINVAL;
